...
setup_client_service() {
    ...
    # Try moonfrp-frpc pattern first (unified naming), then moonfrp-client (legacy), then frpc (legacy)
    if [[ -f "$CONFIG_DIR/moonfrp-frpc${config_suffix}.toml" ]]; then
        config_file="$CONFIG_DIR/moonfrp-frpc${config_suffix}.toml"
    elif [[ -f "$CONFIG_DIR/moonfrp-client${config_suffix}.toml" ]]; then
        config_file="$CONFIG_DIR/moonfrp-client${config_suffix}.toml"
    elif [[ -f "$CONFIG_DIR/frpc${config_suffix}.toml" ]]; then
        config_file="$CONFIG_DIR/frpc${config_suffix}.toml"
    else
        log "ERROR" "Client configuration not found: moonfrp-frpc${config_suffix}.toml, moonfrp-client${config_suffix}.toml, or frpc${config_suffix}.toml"
        return 1
    fi
...
setup_all_services() {
    ...
    # Setup client services (support frpc*.toml, moonfrp-client*.toml, and moonfrp-frpc*.toml)
    local client_configs=($(find "$CONFIG_DIR" -maxdepth 1 \( -name "frpc*.toml" -o -name "moonfrp-client*.toml" -o -name "moonfrp-frpc*.toml" \) -type f 2>/dev/null | sort))
    for config in "${client_configs[@]}"; do
        local config_name=$(basename "$config" .toml)
        local suffix=""
        if [[ "$config_name" == frpc* ]]; then
            suffix="${config_name#frpc}"
        elif [[ "$config_name" == moonfrp-client* ]]; then
            suffix="${config_name#moonfrp-client}"
        elif [[ "$config_name" == moonfrp-frpc* ]]; then
            suffix="${config_name#moonfrp-frpc}"
        fi
...
get_service_config_path() {
    ...
    if [[ "$service_name" == ${CLIENT_SERVICE_PREFIX}* ]]; then
        ...
        # Check for moonfrp-frpc pattern first (unified naming), then moonfrp-client (legacy), then frpc (legacy)
        if [[ -f "$CONFIG_DIR/moonfrp-frpc${suffix}.toml" ]]; then
            echo "$CONFIG_DIR/moonfrp-frpc${suffix}.toml"
        elif [[ -f "$CONFIG_DIR/moonfrp-client${suffix}.toml" ]]; then
            echo "$CONFIG_DIR/moonfrp-client${suffix}.toml"
        else
    ...
    while true; do
        ...
        case "$choice" in
            ...
            3)
                ...
                ;;
            4)
                if [[ ! -f "$CONFIG_DIR/frps.toml" ]] && [[ $(find "$CONFIG_DIR" \( -name "frpc*.toml" -o -name "moonfrp-client*.toml" -o -name "moonfrp-frpc*.toml" -o -name "visitor*.toml" \) -type f 2>/dev/null | wc -l) -eq 0 ]]; then
